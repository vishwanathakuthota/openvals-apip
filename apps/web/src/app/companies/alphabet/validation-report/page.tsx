...
import { fetchAlphabetValidationReport } from "@/lib/api";

export const metadata: Metadata = {
  title: "Alphabet Validation Report | APIP",
  description: "OpenVals gold standard validation workspace for Alphabet AI economics evidence."
};

export default async function AlphabetValidationReportPage() {
  const report = await fetchAlphabetValidationReport();
  return (
    <div className="grid gap-8">
      <header className="grid gap-4 border-b border-border pb-6">
        ...
        <div className="grid gap-2">
          <h1 className="text-4xl font-semibold">Alphabet Validation Report</h1>
          <p className="max-w-3xl text-muted-foreground">
            Source-backed validation workspace for Alphabet revenue, AI revenue, AI investment,
      ...
      <section className="grid gap-3 rounded-lg border border-border bg-card p-5">
        ...
        <div className="flex flex-wrap gap-2">
          {[
            ["/companies/alphabet/evidence-timeline", "Evidence Timeline"],
            ["/companies/alphabet/source-lineage", "Source Lineage"],
            ["/companies/alphabet/openvals-score", "OpenVals Score"],
            ["/companies/alphabet/trust-report", "Trust Report"]
          ].map(([href, label]) => (
            <Link
