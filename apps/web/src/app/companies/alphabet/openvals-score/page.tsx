...
import { fetchAlphabetOpenValsScore } from "@/lib/api";

export const metadata: Metadata = {
  title: "Alphabet OpenVals Score | APIP",
  description: "Alphabet company-level OpenVals Score from Gold Standard validation."
};

export default async function AlphabetOpenValsScorePage() {
  const score = await fetchAlphabetOpenValsScore();
  return (
    <div className="grid gap-6">
      <header className="grid gap-2 border-b border-border pb-6">
        ...
        {score.gold_standard_label ? <Badge>{score.gold_standard_label}</Badge> : null}
